...
require 'config/fungsi.php'; // Ensure the function file is included
...
// Process form when room data is submitted via POST method
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['alamat_222271'])) {
    $alamat = mysqli_real_escape_string($db, $_POST['alamat_222271']);
    $harga = (int)$_POST['harga_222271'];
    ...
    $rating = (float)$_POST['rating_222271'];
    ...
    if (!is_numeric($harga) || $harga <= 0) {
        echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Harga harus lebih besar dari 0 dan berupa angka!',
    ...
    if (!is_numeric($rating) || $rating <= 0 || $rating > 5) {
        echo "<script>
                Swal.fire({
                    ...
                    text: 'Rating harus berada di antara 1 hingga 5!',
                    showClass: {popup: 'animate__animated animate__shakeX'},
                    hideClass: {popup: 'animate__animated animate__fadeOutUp'}
                });
              </script>";
    ...
    // Validate room size (e.g. 3x4)
    $ukuranPattern = '/^\d+x\d+$/';
    if (!preg_match($ukuranPattern, $ukuran)) {
        echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Ukuran tidak valid!',
                    text: 'Format ukuran kamar harus berupa angka, misal: 3x4!',
                    showClass: {popup: 'animate__animated animate__shakeX'},
                    hideClass: {popup: 'animate__animated animate__fadeOutUp'}
                });
              </script>";
    ...
    if (isset($_FILES['foto_222271'])) {
        foreach ($_FILES['foto_222271']['name'] as $i => $fileName) {
            $fileTmp = $_FILES['foto_222271']['tmp_name'][$i];
            $uniqueName = uniqid() . '_' . basename($fileName);
            $fotoPath = 'uploads/' . $uniqueName;

            // No restriction on file type; accept all file extensions
            if (move_uploaded_file($fileTmp, $fotoPath)) {
                ...
                echo "<script>
                        Swal.fire({
                            ...
                            text: 'Ada kesalahan saat mengupload gambar. Silakan coba lagi.',
                            showClass: {popup: 'animate__animated animate__shakeX'},
                            hideClass: {popup: 'animate__animated animate__fadeOutUp'}
                        });
                      </script>";
    ...
    // Combine photo paths into a string
    $fotoPathsString = implode(',', $fotoPaths);

    // Insert data into database
    $sql = "INSERT INTO kamar_222271 
            (alamat_222271, harga_222271, deskripsi_222271, tanggal_tersedia_222271, fasilitas_222271, foto_222271, ukuran_222271, rating_222271) 
    ...
    if ($db->query($sql) === TRUE) {
        echo "<script>
                Swal.fire({
                    ...
                    text: 'Data kamar berhasil disimpan ke sistem.',
...
$sql = "SELECT * FROM pengguna_222271"; // Replace with your actual query
$result = mysqli_query($db, $sql); // Assuming $db is your database connection

// Check if data was fetched
if ($result) {
    $rows = mysqli_fetch_all($result, MYSQLI_ASSOC); // Fetch data as an associative array
} else {
    $rows = []; // Set $rows to an empty array if the query fails
}
                                ...
                                <div class="card shadow-lg border-0">
                                    <div class="card-header bg-gradient-primary text-white text-center py-3">
                                        <h4 class="card-title mb-0">
                                            <i class="fas fa-users me-2"></i> Semua Pengguna yang Telah Bergabung dan Aktif di Sistem
                                        </h4>
                                    </div>
                                    <div class="card-body">
                                                        ...
                                                        <tr class="align-middle">
                                                            ...
                                                            <td class="text-center">
                                                                <div class="position-relative">
                                                                    <img
                                                                        src="<?php echo htmlspecialchars($row['foto_222271']); ?>"
                                                                        alt="Foto Profil"
                                                                        class="img-thumbnail rounded-circle shadow-sm hover-scale"
                                                                        style="width: 50px; height: 50px; object-fit: cover;">
                                                                    <span class="status-indicator bg-success"></span>
                                                                </div>
                                                            </td>
                                                            <td class="text-start fw-bold"><?php echo htmlspecialchars($row['nama_222271']); ?></td>
                                                            <td class="text-start text-muted"><?php echo htmlspecialchars($row['email_222271']); ?></td>
                                                            <td class="text-center"><?php echo htmlspecialchars($row['nomorTelepon_222271']); ?></td>
                                                            <td class="text-center">
                                                                <span class="
                        <?php echo htmlspecialchars($row['role_222271']) === 'Admin' ? 'bg-danger' : 'bg-primary'; ?> text-white px-3 py-2 shadow-sm">
                                                                    <?php echo htmlspecialchars($row['role_222271']); ?>
                                                                </span>
                                                            </td>
                                                        </tr>
                                                    <?php endforeach; ?>
