<?php
session_start(); // Pastikan session dimulai pertama kali

require 'config/fungsi.php'; // Pastikan fungsi.php di-include setelah session_start()
...
$nama = $_SESSION['user']['name'] ?? ''; // Null coalescing operator jika tidak ada data
$role = $_SESSION['user']['role'] ?? ''; // Null coalescing operator jika tidak ada data

// Query untuk mengambil data pengguna dengan role 'user'
$rows = query("SELECT * FROM pengguna_222271 WHERE role_222271 = 'user'");

// Mengambil jumlah penghuni dari database
...
$totalTransaksi = $totalTransaksiQuery[0]['total'] ?? 0; // Default ke 0 jika tidak ada data

// Proses login pengguna (untuk login pertama kali)
...
// Mengambil data penyewaan kos
$rowsPenyewaan = query("SELECT * FROM penyewaan_kos_222271");

// Mengambil total penghuni
$totalPenghuniQuery = query("SELECT COUNT(*) AS total FROM pengguna_222271");
                                ...
                                <div class="card shadow-sm mb-4">
                                    <div class="card-body">
                                        <div class="active-member">
                                            <!-- Judul -->
                                            <h4 class="card-title text-left mb-4">Data Kategori Kamar</h4>
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                ...
                                                <!-- Tombol Tambah Data -->
                                                <a href="addKamar.php" class="btn btn-primary">
                                                        ...
                                                        <?php if (!empty($data_kamar)) : ?>
                                                            <?php foreach ($data_kamar as $index => $row) : ?>
                                                                <tr>
                                                                    ...
                                                                    <td><?= htmlspecialchars(number_format($row['harga_222271'], 2, ',', '.')); ?></td>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        if (!empty($foto_path) && file_exists($full_path)) {
                                                                            echo '<img src="' . $full_path . '" alt="Foto Kamar" style="width: 150px; border-radius: 5px; border: 1px solid #ddd; padding: 5px;">';
                                                                        } else {
                                                                            echo '<img src="uploads/placeholder.jpg" alt="Foto Tidak Tersedia" style="width: 150px;">';
                                                                        }
                                                                        ?>
                                                                    </td>
                                                                    ...
                                                                    <td class="text-center">
                                                                        <a href="editkamarkost3.php?id=<?= htmlspecialchars($row['id_222271']); ?>" class="btn btn-warning btn-sm">
                                                                            <i class="fas fa-edit"></i> Edit
                                                                        </a>
                                                                        <a href="hapus_data.php?id=<?= htmlspecialchars($row['id_222271']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
